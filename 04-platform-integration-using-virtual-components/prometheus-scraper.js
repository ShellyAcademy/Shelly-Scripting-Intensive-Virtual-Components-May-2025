...
const monitored_inputs = ["input:2"]

const CALIBRATION_FACTOR = 4.5
...
function httpServerHandler(request, response) {
    response.body = [
        ...
        monitored_inputs.map(function (input_string_id) {
            return generateMetricsForInput(input_string_id)
        }).join("")
    ].join("")
    response.code = 200;
...
function generateMetricsForInput(string_id) {
    const input = Shelly.getComponentStatus(string_id);
    return [
        printPrometheusMetric("input_frequency", TYPE_GAUGE, [promLabel("input", input.id)], "Frequency", input.freq / CALIBRATION_FACTOR),
    ].join("");
}
